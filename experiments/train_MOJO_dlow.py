...
def recon_loss(Y_g, Y):
    ...
    dist = diff.abs().sum(dim=-1).sum(dim=0)
...
def loss_function(Y_g, Y, a, b):
    ...
    if not type(a) == list:
        ...
        for ff in range(n_freq):
            KLD+= dlow.get_kl(a[ff], b[ff]) / a[ff].shape[0] / n_freq
    # joint loss
...
def train(epoch):
    ...
    while batch_gen.has_next_rec():
        ...
        if n_freq is not None:
            Z_highfreq = torch.randn((t_pred-n_freq, Z.shape[1], Z.shape[2]), device=Z.device)
            Z = torch.cat([Z, Z_highfreq],dim=0) #[freq, b*nk, d]
        ...
        for i in range(args.iter, cfg.num_dlow_epoch):
            ...
            if cfg.save_model_interval > 0 and (i + 1) % cfg.save_model_interval == 0:
                with to_cpu(dlow):
                    ...
                    model_cp = {'model_dict': dlow.state_dict()}
